...
ELSE SUM(b.slots * f.membercost) END AS tot_cost
FROM Members  AS m
INNER JOIN Bookings AS b
USING (memid) 
INNER JOIN Facilities AS f
...
USING (memid)
GROUP BY f.name
HAVING tot_cost > 1000
        

/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
SELECT (firstname || " "|| surname) AS membername,
        (SELECT (m1.firstname || " "|| m1.surname) AS recommended 
            FROM members m1
...
SELECT f.name, (firstname || " "|| surname) AS membername
        FROM Members AS m
        INNER JOIN Bookings AS b
        USING (memid) 
        INNER JOIN Facilities AS f
